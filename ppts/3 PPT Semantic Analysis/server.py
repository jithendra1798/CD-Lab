# Design and implement a server based text based conferencing system with the following features.

#  • A server will maintain the accounts of the various users, through a simple user registration process. 

# • Users can log onto the system whenever they want. 

# • A designated user will be authorized to initiate a conference, view the users recently logged on.

#  • During the conference, any user can type some text at his/her terminal which can be seen by all other users participating in the conference.

# • During the conference, users can also share files by uploading them to the server, which can be downloaded by the other users
...
def pychatroom(client, username, groupname):
	while True:
...
def handshake(client):
	...
	if groupname in groups:
		if username in groups[groupname].allMembers:
			...
			groups[groupname].broadcast(username+" has requested to join the group.","pychatroom")
			client.send(b"/wait")
			print("Join Request:",username,"| Group:",groupname)
		threading.Thread(target=pychatroom, args=(client, username, groupname,)).start()
	else:
...
def main():
	...
	print("pychatroom Server running")
	while True:
